const getHWID = ({hardwareOnly=false, enableWebgl=false}={})=>{
    const devicePixelRatio = +parseInt(window.devicePixelRatio);
    const {appName, appCodeName, appVersion, cookieEnabled, deviceMemory, doNotTrack, hardwareConcurrency, language, languages, maxTouchPoints, platform, product, productSub, userAgent, vendor, vendorSub, } = window.navigator;
    const {width, height, colorDepth, pixelDepth} = window.screen;
    const timezoneOffset = new Date().getTimezoneOffset();
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    const touchSupport = 'ontouchstart'in window;
    const canvas = getCanvasID();
    const webgl = enableWebgl ? getWebglID() : undefined;
    const webglInfo = enableWebgl ? getWebglInfo() : undefined;
    const data = hardwareOnly ? JSON.stringify({
        canvas,
        colorDepth,
        deviceMemory,
        devicePixelRatio,
...
window.hwid = getHWID({enableWebgl:true})